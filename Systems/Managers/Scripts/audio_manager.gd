...
func _apply_audio_settings(s: BattleAudioSettings) -> void:
	if s.use_dungeon_as_base:
		_apply_dungeon_bus_state()
	bgm_lowpass.cutoff_hz   = s.lowpass_cutoff_hz
	bgm_highpass.cutoff_hz  = s.highpass_cutoff_hz
	bgm_reverb.room_size    = s.reverb_room_size
	bgm_distortion.drive    = s.distortion_drive
	bgm_distortion.pre_gain = s.distortion_pre_gain


func play_battle_track(track: BattleTrack):

	stop_bgm() # ensure clean state
	if track.audio_settings != null:
		_apply_audio_settings(track.audio_settings)
	else:
...
func stop_bgm():
	# Kill any in-flight loop crossfade so it can't interfere with the next track.
	if is_instance_valid(_loop_tween):
		_loop_tween.kill()
		_loop_tween = null
	_looping = false
	# Restore the bus volume in case a loop fade left it at -80.
	AudioServer.set_bus_volume_db(bgm_bus, 0.0)
	bgm_base.stop()
	for player in bgm_layers.values():
		player.stop()
...
func _on_bgm_finished() -> void:
	...
	_loop_tween.tween_method(
		func(v): AudioServer.set_bus_volume_db(bgm_bus, v),
		normal_vol, -80.0, LOOP_FADE_TIME)
	await _loop_tween.finished
	if not _looping:
		return  # stop_bgm() was called mid-fade — abort

...
static func capture(run_state: RunState, pos: Vector2i, dungeon_phase: int = 0, grid_segno_pos: Vector2i = Vector2i(-999,-999), past_positions: Array = []) -> SegnoSnapshot:
	...
	snap.money         = run_state.money
	snap.reroll_charges = run_state.reroll_charges
	snap.road_tiles    = run_state.road_tiles_remaining
...
func restore_into(run_state: RunState) -> void:
	...
	run_state.money               = money
	run_state.reroll_charges      = reroll_charges
	run_state.road_tiles_remaining = road_tiles

	...
	for effect in sd.effects:
		...
		match effect.effect_type:
			...
			SkillEffect.EffectType.HEAL:
				for t in targets:
					if is_instance_valid(t) and t.is_alive():
						...
						log_msg("%s restores %d HP to %s." % [name, amt, t.name])
			...
			SkillEffect.EffectType.SELF_HEAL:
				...
				log_msg("%s recovers %d HP." % [name, amt])
					...
					if is_instance_valid(t) and t.is_alive():
						...
						if is_alive() and t != null:
							...
							if dealt > 0:
								...
								emit_signal("hp_changed")
								log_msg("%s drains %d HP from %s." % [name, heal, t_name])
						else:
